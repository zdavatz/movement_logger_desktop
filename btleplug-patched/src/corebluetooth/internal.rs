...
pub enum CoreBluetoothMessage {
    ...
    /// macOS reconnect (retrieve-by-identifier, no scan). Ask CoreBluetooth
    /// for a peripheral it already knows by UUID and re-insert it into the
    /// internal maps via the discovery path, so a subsequent ConnectDevice
    /// finds it. Mirrors iOS `retrievePeripherals(withIdentifiers:)`.
    /// (Vendored patch — not in upstream btleplug.)
    RetrievePeripheral {
        peripheral_uuid: Uuid,
        future: CoreBluetoothReplyStateShared,
    },
...
impl CoreBluetoothInternal {
    ...
    /// Vendored patch: handle `RetrievePeripheral`. Ask CoreBluetooth for the
    /// peripheral it already knows by identifier (no scan / no advertising
    /// required) and, if found, route it through the *exact* discovery
    /// insertion path (`on_discovered_peripheral`) so Map A (`self.peripherals`)
    /// is repopulated and a `DeviceDiscovered` is dispatched (→ the public
    /// AdapterManager map on the Adapter event task). After this the ordinary
    /// `ConnectDevice` path finds the id in Map A and issues
    /// `connectPeripheral:options:` — the macOS equivalent of iOS's
    /// `retrievePeripherals(withIdentifiers:)` + pending connect. This is the
    /// only way to reconnect a peripheral CoreBluetooth has stopped returning
    /// in scan results after a connect/disconnect cycle.
    ///
    /// `on_discovered_peripheral` guards on `contains_key`, so a still-known
    /// peripheral only emits `DeviceUpdated` — no double-add.
    async fn on_retrieve_peripheral(
        &mut self,
        peripheral_uuid: Uuid,
        fut: CoreBluetoothReplyStateShared,
    ) {
        // NSUUID keyed exactly as CoreBluetooth stores it; from_bytes is
        // infallible and round-trips with btleplug's UUIDString().parse().
        let nsuuid = NSUUID::from_bytes(peripheral_uuid.into_bytes());
        let ids = NSArray::from_vec(vec![nsuuid]);
        // SAFETY: CBPeer + CBPeripheral features are enabled; `ids` is a valid
        // NSArray<NSUUID>. Returns peripherals CB already knows by identifier;
        // does not touch the radio. Runs on the CBCentralManager's serial
        // dispatch queue thread, same as every other manager call here.
        let found = unsafe { self.manager.retrievePeripheralsWithIdentifiers(&ids) };

        if let Some(cbperipheral) = found.into_iter().next() {
            trace!("retrievePeripheralsWithIdentifiers hit for {peripheral_uuid}");
            self.on_discovered_peripheral(cbperipheral, None).await;
            fut.lock().unwrap().set_reply(CoreBluetoothReply::Ok);
        } else {
            trace!("retrievePeripheralsWithIdentifiers empty for {peripheral_uuid}");
            fut.lock().unwrap().set_reply(CoreBluetoothReply::Err(
                "CoreBluetooth has no peripheral with that identifier".into(),
            ));
        }
    }
            ...
            adapter_msg = self.message_receiver.select_next_some() => {
                ...
                match adapter_msg {
                    ...
                    CoreBluetoothMessage::RetrievePeripheral{peripheral_uuid, future} => {
                        self.on_retrieve_peripheral(peripheral_uuid, future).await;
                    }
