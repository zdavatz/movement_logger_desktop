...
impl PeripheralId {
    /// The CoreBluetooth peripheral identifier (host-stable UUID) this id
    /// wraps. Used by the vendored retrieve-by-identifier reconnect path
    /// (`Adapter::add_peripheral` → `RetrievePeripheral`).
