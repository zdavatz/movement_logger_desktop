...
// Patched for App Store compliance: NSInteger + AnyObject were only used
// by the now-removed `_CGSSetWindowBackgroundBlurRadius` extern. Path
// patches don't get the registry-dep --cap-lints=warn treatment, so an
// unused import here trips RUSTFLAGS=-Dwarnings in CI.
...
extern "C" {
    ...
    // Private blur APIs removed for App Store compliance (winit #4538)
}
