...
- State lives in a local SQLite DB at `~/.movementlogger/sync.db` (Windows `%USERPROFILE%\.movementlogger\sync.db`) — `sync_db.rs`, `rusqlite` with the `bundled` feature so there's no system libsqlite (same self-contained rationale as `rustls-tls`). Anchored to `$HOME`, **not** to the user-selectable "Save to" folder, so changing that folder doesn't orphan history and re-pull everything.
