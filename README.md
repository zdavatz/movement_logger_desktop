...
For a race (wingfoil, pumpfoil, …) every rider's phone can stream its live GPS position to this app: the **Race** tab listens on a UDP port (default 47777) and plots each rider as a coloured dot with an accuracy circle and trail on an OpenStreetMap or satellite view, with a rider list showing speed, source, accuracy, satellites and battery. Riders enable **Race mode** in the Android app (GPS tab — u-blox dongle or the phone's own GPS) or the iOS app (GPS tab — iPhone GPS, or the Apple Watch, which even works phone-free on the venue WiFi) and enter the `ip:port` the Race tab shows when it starts listening. Every received fix is also appended to `~/.movementlogger/race/race_<timestamp>.csv` for post-race analysis.

**Over cellular (real race days):** a phone/laptop hotspot only reaches ~30–80 m outdoors, so for a 500 m course let the riders use mobile data and run the **race relay** on any Linux server with a public IP (fine next to an existing web server — Apache owns TCP 80/443, the relay owns UDP 47777). Every release ships `race-relay-vX.Y.Z-x86_64-unknown-linux-musl.tar.gz`: one static binary + a systemd unit — `cp` both, `systemctl enable --now race-relay`, open UDP 47777 in the firewall, point a DNS name at the server. Riders then enter the relay's `host:port` in the same Race card, and the desktop ticks **"Via relay"** in the Race tab. No port forwarding is needed on either side.
